...
\begin{array}{rcll}
	...
	\sigma'(z) & = & \sigma(z) \odot (1 - \sigma(z)) & \text{Sigmoid Derivative}\\
	&&\\
	a^{(l)} & = & \sigma\{a^{(l-1)}*[\theta^{(l-1)}]^T\} & \text{Activation/Output of layer l}\\
	&&\\
		Y^{(i)}_{k=1} & = & \begin{bmatrix} 
		0 \\
		0 \\
		\vdots \\
		1 \\
		\vdots \\
		0 \end{bmatrix} & \text{One-Hot Form of Classification Result}\\
	&&\\
	J(\theta) & = & -\frac{1}{m}\sum_{i=1}^{m}\sum_{j=1}^{||L||}[Y \odot log(a^{(L)}) + (1-Y) \odot log(1-a^{(L)})] & \text{Cost function of NN used for Backpropogation,}\\
	...
	&&\\
	\delta^{(l)} & = & \widehat{\delta}^{(l+1)}\widehat{\Theta}^{(l)} \odot \sigma'(a^{(l)}\Theta^{(l)}) & \text{Error of layer l}\\
	&&\\
